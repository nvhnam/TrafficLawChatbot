...
class LegalSemanticSplitter:
    ...
    def _process_accumulated_dieu(self, current_dieu_lines, doc_name, chunks, current_phan, current_chuong, current_muc):
        ...
        dieu_match = re.search(r'^(?:#+\s*)?(?:Điều|Dieu|Đieu)\s+(\d+[a-zA-Z]*)[.,]\s*(?:[\*\#\s]*)(.*?)(?:[\*\#\s]*)(?=\n|$)', dieu_text, flags=re.IGNORECASE)
        ...
        for khoan_text in khoan_splits[1:]:
            ...
            is_amendment = bool(re.search(r'^\d+\.\s+(Sửa đổi|Sua doi|Bổ sung|Bo sung|Bãi bỏ|Bai bo|Thay thế|Thay the)', khoan_text, flags=re.IGNORECASE))
            diem_splits = re.split(r'\n(?=[a-zđ]\)\s+)', '\n' + khoan_text, flags=re.IGNORECASE)
            ...
            if len(diem_splits) == 1:
                ...
                for diem_text in diem_splits[1:]:
                    ...
                    is_sub_amendment = bool(re.search(r'^[a-zđ]\)\s+(Sửa đổi|Sua doi|Bổ sung|Bo sung|Bãi bỏ|Bai bo|Thay thế|Thay the)', diem_text, flags=re.IGNORECASE))
    ...
    def split_text(self, text, doc_name):
        text = re.split(r'\n\s*(?:Nơi nhận:|TM\. CHÍNH PHỦ|VĂN PHÒNG CHÍNH PHỦ XUẤT BẢN)', text, flags=re.IGNORECASE)[0].strip()

        chunks = []

        phu_luc_splits = re.split(r'\n(?=(?:\*\*|#+\s*)?(?:Phụ lục|Phu luc)\s+)', text, flags=re.IGNORECASE)
        ...
        for line in lines:
            ...
            if re.match(r'^PHẦN\s+(THỨ\s+)?[A-ZĐ]+', line_stripped, flags=re.IGNORECASE):
                current_phan = line_stripped
                continue

            if re.match(r'^Chương\s+[IVXLCDM]+', line_stripped, flags=re.IGNORECASE):
                current_chuong = line_stripped
                current_muc = None
                continue

            if re.match(r'^Mục\s+\d+', line_stripped, flags=re.IGNORECASE):
                current_muc = line_stripped
                continue

            if re.match(r'^(?:#+\s*)?(?:Điều|Dieu|Đieu)\s+\d+[a-zA-Z]*[.,]', line_stripped, flags=re.IGNORECASE):
        ...
        for pl_text in phu_luc_texts:
            ...
            pl_match = re.search(r'^(?:\*\*|#+\s*)?((?:Phụ lục|Phu luc)\s+[A-Za-z0-9]+)', pl_text, flags=re.IGNORECASE)
