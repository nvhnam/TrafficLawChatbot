...
from backend.config import (
    ...
    INGESTION_MIN_INTERVAL_SECONDS,
    INGESTION_MAX_CALLS_PER_MINUTE,
)
...
class PdfToMarkdownConverter(object):
    ...
    def _call_api_with_retry(self, chunk_bytes):
            ...
            with open(temp_data_file, "a", encoding="utf-8") as f_temp:
                for chunk_bytes in remaining_chunks:
                    ...
                    if result_text == "QUOTA_EXCEEDED":
                        logger.critical(
                            "Markdown conversion for '%s' stopped at chunk %d/%d: all fallback models exhausted.",
                    ...
                    logger.info(
                        "Markdown chunk %d/%d converted for '%s' (%d chars).",
                        current_count, total_chunks, file_identifier, len(result_text),
                    )
